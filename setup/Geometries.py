...
class Geometries:
    def __init__(self,parent,mainmesh=True,hfs=None):
        ...
        if hfs==None: self.height_fields = get_random_height_fields(self.parent.dim,self.parent.noc) #Initiate a random joint geometry
        else: self.height_fields = hfs
        if self.mainmesh: self.select = Selection(self)
        #if self.mainmesh: self.fab = Fabrication(self)
    ...
    def voxel_matrix_from_height_fields(self, first=False):
        ...
        if self.mainmesh: self.eval = Evaluation(self.voxel_matrix,self.parent)
        if self.mainmesh and not first: self.parent.update_suggestions()
